...
def _get_bndbox_coor(jpg_Label_file, folder, type_name):
    ...
    crop_start_point = crop_list[0]
    crop_end_point = crop_list[-1]
    ...
    # cv2.imwrite('check.jpg', resized_crop_img)
...
def branch2_real_crop_xml(jpg_Label_file, folder_num, type_name, twidth, theight):
    """
    ...
    filename.text = jpg_Label_file
    path = ET.SubElement(annotation, 'path')
    path.text = save_crop_ROI_path + jpg_Label_file  # 'D:\Mammograph\ROI_training_dataset\JPEGImages/'
    source = ET.SubElement(annotation, 'source')
    database = ET.SubElement(source, 'database')
    database.text = 'Unknown'
    size = ET.SubElement(annotation, 'size')
